...
| [Nominatim public-service policy](https://operations.osmfoundation.org/policies/nominatim/) and [OpenStreetMap credits](https://www.openstreetmap.org/copyright) | Place/address search, reverse lookup, and road names through a Nominatim-compatible provider. | Enabled on the live app backend (`chatty-puffin-875`) with `GEOCODING_BASE_URL=https://nominatim.openstreetmap.org` after operator approval on September 21, 2026. Live ZIP searches for Denver and Boston were verified. Public Nominatim requires an explicit operator choice, attribution, an application-wide maximum of one request per second, and no autocomplete. Successful results are cached for seven days; empty searches for one hour. See [geocoding configuration](geocoding.md). |
...
| --- | --- | --- |
| City and County of Denver / Denver Police Department: [Traffic Accidents, ArcGIS layer 325](https://services1.arcgis.com/zdB7qR0BtYrg0Xpl/ArcGIS/rest/services/ODC_CRIME_TRAFFICACCIDENTS5YR_P/FeatureServer/325) | Incident identifiers, coordinates, occurrence dates, bicycle/pedestrian involvement, fatalities, and serious injuries. | Connected. Adapter requests the previous five calendar years plus the current year to date. Publisher says records are updated weekdays and can be revised. The backend archives twelve calendar months, deduplicates incident IDs, and refreshes recent months daily. Older source records remain available only through the publisher. Other injury severity remains unknown; involvement flags do not identify who was injured. |
| Chicago Police Department / Chicago Data Portal: [Traffic Crashes — Crashes](https://data.cityofchicago.org/Transportation/Traffic-Crashes-Crashes/85ca-t3if) | Crash identifiers, coordinates, dates, injury/fatality totals, most severe injury, and first crash type. | Connected; dataset ID `85ca-t3if`. The backend archives twelve calendar months using paginated monthly imports. Mode is inferred from first crash type. The archive keeps unmatched severity values unknown. Legacy direct-fetch mode labels unmatched values minor; neither is an independently verified clinical severity classification. |
| NYPD / NYC Open Data: [Motor Vehicle Collisions — Crashes](https://data.cityofnewyork.us/Public-Safety/Motor-Vehicle-Collisions-Crashes/h9gi-nx95) | Collision identifiers, coordinates, dates, and injury/fatality counts by road-user group. | Connected; dataset ID `h9gi-nx95`. The backend archives twelve calendar months using paginated monthly imports. Injury counts do not establish injury severity; nonfatal records are classified as unknown. Modes derive from casualty fields and are not a complete inventory of involved road users. |

The live backend archive serves map bounds/date queries and city-wide monthly totals. Map reads are capped at 5,000 scanned records and marked partial when capped; timeline totals are independent of this display limit. Imports omit unusable coordinates and disclose skipped-row counts. Browser results are cached for five minutes; Refresh crash data bypasses that cache. See [one-year crash archive](crash-history.md) for retention, scheduling, atomic publication, and limitations. The direct provider adapters remain available only when no backend is configured. Municipal datasets do not establish coverage of surrounding municipalities. The app exposes source failures and partial-result warnings. These layers are records of reported crashes, not a complete measure of exposure or street safety.

Implementation: `src/features/safety-data/api/` and `src/features/safety-data/CrashCoverageStatus.tsx`.

### Live crash-feed refresh — September 21, 2026

Fetched fresh publisher metadata and the five newest source records, then ran the existing application adapters against city-wide bounds for August 22–September 21, 2026. These were live read-only fetches, separate from the mocked test suite.

| Source | Latest occurrence returned by publisher | Adapter result for the requested date window |
| --- | --- | --- |
| Chicago | September 21, 2026 | 1,000 mapped records; capped and explicitly partial. |
| Denver | September 4, 2026 | 152 mapped incidents; no pagination-limit warning. |
| NYC | June 11, 2026 | Zero records in the requested window. Publisher metadata says rows last updated June 15, 2026; this is upstream staleness, not evidence of no recent crashes. |

**This check preceded the archive rollout described above.** At the time there was no shared crash-data import or persistent backend snapshot to refresh. Clients fetch the municipal APIs on demand; completed queries remain in that browser's memory for five minutes. A page reload clears that memory cache and the next enabled crash-layer request fetches fresh data. Cache expiry alone does not initiate polling. This refresh did not modify municipal records or community reports, and cannot make the publishers' lagging data newer.
